...
// Gestion menu Homme / Femme
const hommeToggle = document.getElementById("hommeToggle");
const femmeToggle = document.getElementById("femmeToggle");
const hommeSubmenu = document.getElementById("hommeSubmenu");
const femmeSubmenu = document.getElementById("femmeSubmenu");
const backToMainMenu2 = document.getElementById("backToMainMenu2");

function resetAllSubmenus() {
  if (mobileMenuMain) mobileMenuMain.classList.remove("is-hidden");
  if (pretaporterSubmenu) pretaporterSubmenu.classList.add("is-hidden");
  if (hommeSubmenu) hommeSubmenu.classList.add("is-hidden");
  if (femmeSubmenu) femmeSubmenu.classList.add("is-hidden");
}

if (hommeToggle) hommeToggle.addEventListener("click", () => {
  if (!mobileMenuMain || !hommeSubmenu) return;
  mobileMenuMain.classList.add("is-hidden");
  hommeSubmenu.classList.remove("is-hidden");
  if (femmeSubmenu) femmeSubmenu.classList.add("is-hidden");
});

if (femmeToggle) femmeToggle.addEventListener("click", () => {
  if (!mobileMenuMain || !femmeSubmenu) return;
  mobileMenuMain.classList.add("is-hidden");
  femmeSubmenu.classList.remove("is-hidden");
  if (hommeSubmenu) hommeSubmenu.classList.add("is-hidden");
});

if (backToMainMenu2) backToMainMenu2.addEventListener("click", resetAllSubmenus);
...
const USERS_KEY = "msprestige_users";
const CURRENT_USER_KEY = "msprestige_current_user";
const CART_KEY = "msprestige_cart";
const PROMO_KEY = "msprestige_promo";
    ...
    <div class="account-layout">
      <div class="account-box">
        <div class="account-box-head">
          <h2>Mon espace</h2>
          <p>Vous êtes connecté à votre compte MS Prestige.</p>
        </div>
  ...
  if (cart.length === 0) {
    cartContainer.innerHTML = `
      <div class="page-card">
        ...
        <p>Ajoutez des pièces MS Prestige pour commencer votre sélection.</p>
      ...
      return `
        <div class="cart-item" data-id="${item.id}">
          ...
          <div class="cart-item-info">
            <h2>${item.name}</h2>
            <p>${item.category || "Collection MS Prestige"}</p>
...
function initDynamicProductPage() {
  ...
  const finalDescription =
    description || `Une pièce premium ${brand || ""} sélectionnée par MS Prestige.`;
  ...
  document.title = `${name} | MS Prestige`;
}
